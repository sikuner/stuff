...
#include "com_norco_utils_SerialPortJNI.h"

#include "android/log.h"
static const char *TAG = "JNI C++ file SerialPort485";
 ...
 */JNIEXPORT jobject JNICALL Java_com_norco_utils_SerialPortJNI_openDev(
	...
	sprintf(dev, "/dev/ttymxc%d", devnum);

	LOGD("NICK OPENDEV==>OPEN %s", dev);
 ...
 */JNIEXPORT jint JNICALL Java_com_norco_utils_SerialPortJNI_readBytes(
	...
	int byteGetCount = 0;
	fd_set read_fd_set;
	jbyte* jBuffer = env->GetByteArrayElements(readbuf, JNI_FALSE);
	...
	while (byteRemains > 0) {
		...
		if (result > 0) {
			...
			env->ReleaseByteArrayElements(readbuf, (jbyte *) jBuffer,
					0);
			return byteGetCount;
		}
	}
	FD_CLR(fd, &read_fd_set);
	env->ReleaseByteArrayElements(readbuf, (jbyte *) jBuffer, 0);
 ...
 */JNIEXPORT jboolean JNICALL Java_com_norco_utils_SerialPortJNI_writeBytes(
	...
	int fd;
	fd = getFileDescriptorID(env, thiz, mfd);
	jbyte* jBuffer = env->GetByteArrayElements(writebuf, JNI_FALSE);
	jint result = write(fd, jBuffer, (size_t) length);
	env->ReleaseByteArrayElements(writebuf, jBuffer, 0);
 ...
 jint getFileDescriptorID(JNIEnv *env, jobject thiz, jobject jfd) {
 	...
 	jfieldID descriptorID = env->GetFieldID(cFileDescriptor,
 			"descriptor", "I");
 	jint descriptor = env->GetIntField(jfd, descriptorID);
...
JNIEXPORT jobject JNICALL Java_com_norco_utils_SerialPortJNI_open485Dev
  (JNIEnv *env, jobject thiz, jint devnum){
	return Java_com_norco_utils_SerialPortJNI_openDev(env,  thiz,  devnum);
}
...
JNIEXPORT jint JNICALL Java_com_norco_utils_SerialPortJNI_close485Dev(
		JNIEnv *env, jobject thiz, jobject mfd){
	return Java_com_norco_utils_SerialPortJNI_closeDev(env ,thiz, mfd);
}
...
JNIEXPORT jint JNICALL Java_com_norco_utils_SerialPortJNI_set485mod
  (JNIEnv *env, jobject thiz, jint jmode){
	...
	sprintf(dev, "/dev/ttymxc%d", 1);
	
	mod485fd = open( dev, O_RDWR | O_NDELAY );
